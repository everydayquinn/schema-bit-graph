...
PREDICATES = [
    ...
    # ---- Runtime substrate predicates (sim_6502, future sim_jvm, etc.)
    ("ENTRY_STATE",     "prog", "literal", "one",
     "initial register/flag state at the start of a sim run; condensed text",
     ["prog:01_basic ENTRY_STATE 'pc=0x0600, a=0 x=0 y=0 sp=0xff p=0x00, inputs=[]'"]),
    ...
    ("TERMINATED",      "prog", "literal", "one",
     "termination reason: brk@0xHHHH | max-steps | fault@0xHHHH:<exception_type>",
     ["prog:01_basic TERMINATED brk@0x0606", "prog:countdown TERMINATED max-steps"]),
    ("MEM_READ",        "step", "literal", "many",
     "memory read captured during this step: 0xADDR=0xVAL (excludes opcode/operand fetches)",
     ["step:02_interrupt:000003 MEM_READ 0x0200=0x08"]),
    ("MEM_WRITE",       "step", "literal", "many",
     "memory write captured during this step: 0xADDR=0xVAL",
     ["step:01_basic:000002 MEM_WRITE 0x0200=0x08"]),
    ("INTERRUPT",       "step", "literal", "one",
     "interrupt mark on this step: irq | nmi | brk",
